...
void viewVoterRequest()
{
    ...
    else
    {
        while (fread(&request, sizeof(request), 1, approvedrequest))
        {
            ...
            printf(COLOR_BOLD COLOR_BLUE"Voter with UserId" COLOR_YELLOW "  %d "COLOR_RESET COLOR_BOLD COLOR_BLUE"has requested for VoterId \n"COLOR_RESET"", request.UserId);
            printf(COLOR_BOLD COLOR_BLUE"Press" COLOR_YELLOW" 1 "COLOR_RESET COLOR_BOLD COLOR_BLUE"to Accept Request\n"COLOR_RESET);
            printf(COLOR_BOLD COLOR_BLUE"Press" COLOR_RESET COLOR_BOLD COLOR_YELLOW " 2 " COLOR_RESET COLOR_BOLD COLOR_BLUE"to Reject Request\n"COLOR_RESET);
            printf(COLOR_BOLD"Enter your Choice : \n");
            int choice = 0;
            char ch[10];
            scanf("%s", ch);
            choice = atoi(ch);
            ...
            if (choice == 1)
            {
                printf(COLOR_BOLD COLOR_YELLOW"VoterId is being AutoGenerated Please wait"COLOR_RESET);
                while (sec < 4)
                {
                    if (clock() > stopclock)
                    {
    ...
    else
    {
        ...
        while (fread(&newUser, sizeof(newUser), 1, user))
        {
            if (newUser.UserId == currentUserId)
            {
                ...
                printf(COLOR_BOLD COLOR_GREEN"\nVoterId is Generated successfully!!!\n"COLOR_RESET);
                printf( COLOR_BOLD"\nVoterId =  %s\n", VoterId);
                printf("\n");
...
void officerMenu()
{
    ...
    while (choice != 2)
    {
        ...
        printf(COLOR_BLUE"|------------------------------------------------------------------------|\n"COLOR_RESET);
        printf(COLOR_BLUE"|"COLOR_MAGENTA COLOR_BOLD"                        Electoral Officer Menu                                      "COLOR_RESET COLOR_BLUE "|\n"COLOR_RESET);
        printf(COLOR_BLUE"|------------------------------------------------------------------------|\n"COLOR_RESET);
        printf(COLOR_BOLD"|Please select an option:                                                | \n"COLOR_RESET);
        printf(COLOR_BOLD"%-3s%-70s%-30s\n","|", " 1.  View Voter Request and Generate VoterId","|");
        printf(COLOR_BOLD"|   2. Exit                                                              |\n");
        printf("|------------------------------------------------------------------------|\n");
        printf("\n");
        ...
        default:
            printf("\033[0;31m\n");
            printf(COLOR_BOLD" Invalid Data Enterd Please Enter Valid choise !!! \n");
            printf("\033[0;37m\n");
        }
    }
}
